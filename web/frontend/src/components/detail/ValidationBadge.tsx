...
import { getColor, iconMap } from '../../utils/colors';
...
export function ValidationBadge({ status, reason }: ValidationBadgeProps) {
  const color = getColor(getValidationStatusColor(status));
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium border ${badgeColorMap[color]}`}>
